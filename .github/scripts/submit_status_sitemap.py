...
def get_credentials():
    """Get valid user credentials from storage, user authentication, or service account."""
    creds = None
    
    # First, check if there's a service account key file
    ...
    # Try to find a stored OAuth token
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if os.path.exists('client_secret.json'):
                ...
                if 'CI' in os.environ:
                    print("ERROR: Running in CI environment and no valid credentials found.")
                    print("Please provide a service-account.json file for CI environments.")
                    sys.exit(1)
                ...
                print("ERROR: No credentials found. Please create a client_secret.json or service-account.json file.")
                print("See: https://developers.google.com/search/apis/indexing-api/v3/quickstart")
                sys.exit(1)
        
        # Save the credentials for the next run (only for OAuth credentials)
